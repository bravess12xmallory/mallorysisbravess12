...
client.on("message", (message) => {
    ...
    if (isCommand(message, "!!ticket")) {
        ...
        if (message.guild.channels.exists("name", "tickets" + message.author.id)) return message.channel.send(`**لديك بالفعل تذكرة مفتوحة.**`);
...
يرجى محاولة شرح سبب فتح هذه التذكرة بأكبر قدر ممكن من التفاصيل. سيكون لدينا ** فريق الدعم ** قريباً لمساعدتك..`)
                .setTimestamp();
            c.send({
                embed: embed
            });
    ...
    if (isCommand(message, "close")) {
        if (!message.channel.name.startsWith(`ticket-`)) return message.channel.send(`**لا يمكنك استخدام هذا الامر خارج روم التذاكر**`);

        message.channel.send(`
هل أنت واثق؟ بعد التأكيد ، لا يمكنك عكس هذا الإجراء! \ n للتأكيد ، اكتب \ "/ confirm \`. سوف ينتهي المهلة خلال 10 ثوانٍ ويتم إلغاها.`)
            .then((m) => {
